...
class CoNLLProcessor(object):
    ...
    def get_dev_examples(self, data_dir):
        return list(self._create_examples(self._read_data(os.path.join(data_dir, "dev.txt")), "dev"))
...
def convert_examples_to_features(
    ...
    for example_index, example in enumerate(examples):
        ...
        for n, label in enumerate(example.labels):
            if label.strip() == "O" or n in example.sentence_boundaries:
                if start is not None:
                    entity_labels[(token2subword[start], token2subword[n])] = label_map[cur_type.strip()]
                    start = None
                    cur_type = None
